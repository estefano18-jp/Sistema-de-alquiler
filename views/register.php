<?php
session_start();
require_once '../models/Usuario.php'; // Asegúrate de que la ruta es correcta
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $confirmar_passuser = trim($_POST['confirmar_passuser']);

    // Validación del lado del servidor
    if (empty($nombre) || empty($apellido) || empty($telefono) || empty($nrodocumento) || empty($nomuser) || empty($passuser) || empty($confirmar_passuser)) {
        echo json_encode(['status' => 'Todos los campos son obligatorios.', 'success' => false]);
        exit();
    }

    if (!preg_match("/^\d{8}$/", $nrodocumento)) {
        echo json_encode(['status' => 'El número de documento debe tener 8 dígitos.', 'success' => false]);
        exit();
    }

    if ($passuser !== $confirmar_passuser) {
        echo json_encode(['status' => 'Las contraseñas no coinciden.', 'success' => false]);
        exit();
    }
    ...
    if ($usuario->checkExistingPerson($nrodocumento)) {
        echo json_encode(['status' => 'El número de documento ya está registrado.', 'success' => false]);
        exit();
    }

    // Registrar la persona usando tu procedimiento almacenado
    $idpersona = $usuario->registerPerson($nombre, $apellido, $telefono, $nrodocumento);

    if ($idpersona <= 0) {
        echo json_encode(['status' => 'Error al registrar la persona.', 'success' => false]);
        exit();
    }

    // Registrar el usuario
    $idusuario = $usuario->registerUser($idpersona, $nomuser, password_hash($passuser, PASSWORD_BCRYPT));

    if ($idusuario > 0) {
        echo json_encode(['status' => 'Registro exitoso. Redirigiendo...', 'success' => true]);
    } else {
        echo json_encode(['status' => 'Error al registrar el usuario.', 'success' => false]);
    }
    ...
    <div class="login-container">
        ...
        <form id="form-register" autocomplete="off">
            ...
            <div class="mt-3 text-center">
                <p>¿Ya tienes cuenta? <a href="../index.php" class="btn btn-link">Iniciar sesión</a></p>
            </div>
        </form>
    </div>
